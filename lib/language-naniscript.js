...
class NaniLanguageClient extends AutoLanguageClient {
  ...
  getGrammarScopes () { return [ 'source.nani' ] }
  getLanguageName () { return 'tree_sitter_nani_script' }
  getServerName () { return 'nani_script-language-server' }
  ...
  getServerProcess () {
    ...
    childProcess.on("error", err => {
      atom.notifications.addError(
          "Unable to start Naninovel language server.",
          {
            dismissable: true,
            description: err
          }
    ...
    childProcess.on('exit', exitCode => {
      if (exitCode !== 0 && exitCode != null) {
        atom.notifications.addError('Naninovel language server stopped unexpectedly.', {
          dismissable: true,
