...
class NaniLanguageClient extends AutoLanguageClient {
  ...
  getServerProcess () {
    ...
    const binaryPath = process.platform == 'win32' ? './server/lsp-windows' : './server/lsp-linux';

    const childProcess = cp.spawn(
        binaryPath, ["-meta", "./server/metadata.json", "-logfile", "./server/lsp.log"], { cwd: packageRoot }
    );
